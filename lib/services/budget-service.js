...
module.exports.addBudget = async function (userId, budgetData) {
  await sequelize.transaction(async (t) => {
    ...
    const threshold= new Date(budgetData.thresholdDate)
    if (start>threshold){
        throw new Error('Invalid date. The start date can not after the threshold date')
    }
    ...
    await Budget.create(
      {
        ...
        thresholdDate: budgetData.thresholdDate,
