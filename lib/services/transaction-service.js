...
const {
  checkBudgetAndBuildNotifications,
  checkGoalAndBuildNotifications,
} = require('../utils/notifyAfterTransaction');
...
module.exports.addTransaction = async function (userId, transactionData) {
  return await sequelize.transaction(async (t) => {
    ...
    const notifications = [
      ...
      ...(await checkGoalAndBuildNotifications({
        userId,
        savingGoalId: trans.savingGoalId,
        t,
        addedAmount: Number(trans.amount),
      })),
    ];
...
module.exports.deleteTransaction = async function (userId, transactionId) {
  return await sequelize.transaction(async (t) => {
    ...
    await reverseBalancesUpdate({ userId, trans: deleteTrans.dataValues, t });
    const notifications = [
      ...(await checkBudgetAndBuildNotifications({
        userId,
        budgetId: deleteTrans.dataValues.budgetId,
        t,
      })),
      ...(await checkGoalAndBuildNotifications({
        userId,
        savingGoalId: deleteTrans.dataValues.savingGoalId,
        t,
        addedAmount: Number(deleteTrans.dataValues.amount),
      })),
    ];
...
module.exports.updateTransaction = async function (userId, transactionId, newTransactionInfo) {
  return await sequelize.transaction(async (t) => {
    ...
    await applyBalanceUpdates({ userId, trans: updatedTransaction.dataValues, t });
    const notifications = [
      ...(await checkBudgetAndBuildNotifications({
        userId,
      ...
      ...(await checkGoalAndBuildNotifications({
        userId,
        savingGoalId: updatedTransaction.savingGoalId,
        t,
        addedAmount: Number(updatedTransaction.dataValues.amount),
      })),
    ];
