...
const { checkBudgetAndBuildNotifications, checkGoalAndBuildNotifications } = require("../utils/notifyAfterTransaction");
...
module.exports.addTransaction = async function (userId, transactionData) {
  return await sequelize.transaction(async (t) => {
    ...
    const notifications = [
  ...(await checkBudgetAndBuildNotifications({ userId, budgetId: trans.budgetId, t })),
  ...(await checkGoalAndBuildNotifications({ userId, savingGoalId: trans.savingGoalId, t , addedAmount: Number(trans.amount)})),
];
    return { message: "Transaction is added successfully", notifications };
  });
};
...
async function applyBalanceUpdates({ userId, trans, t }) {
  ...
  if (trans.budgetId) {
    await Budget.increment('usedAmount', {
      by: trans.amount,
  ...
  if (trans.savingGoalId) {
    await SavingGoal.increment('currentAmount', {
      by: trans.amount,
...
module.exports.deleteTransaction = async function (userId, transactionId) {
  return await sequelize.transaction(async (t) => {
    ...
    const notifications = [
  ...(await checkBudgetAndBuildNotifications({ userId, budgetId: deleteTrans.budgetId, t })),
  ...(await checkGoalAndBuildNotifications({ userId, savingGoalId: deleteTrans.savingGoalId, t , addedAmount: Number(trans.amount)})),
];
    await deleteTrans.destroy({ transaction: t });
    return { message: "Transaction is deleted successfully!", notifications };
  });
};
...
module.exports.updateTransaction = async function (userId, transactionId, newTransactionInfo) {
  return await sequelize.transaction(async (t) => {
   ...
   const notifications = [
  ...(await checkBudgetAndBuildNotifications({ userId, budgetId: updatedTransaction.budgetId, t })),
  ...(await checkGoalAndBuildNotifications({ userId, savingGoalId: updatedTransaction.savingGoalId, t, addedAmount: Number(trans.amount) })),
];
    return { message: "Transaction is updated successfully!", notifications };
  });
};
